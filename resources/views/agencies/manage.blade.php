...
<div class="">
  <div class="flex items-center justify-between">
    <div>
      ...
      <p class="text-[var(--c-muted)] mt-1">Gestiona agencias y marca una sola como principal desde el admin interno.</p>
    </div>
  ...
  <div class="bg-[var(--c-surface)] rounded-2xl border border-[var(--c-border)] p-6">
    <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between mb-4">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center gap-2">
        ...
        <select id="filter-primary" class="px-3 py-2 bg-[var(--c-elev)] border border-[var(--c-border)] rounded-lg text-sm">
          <option value="">Principal: Todas</option>
          <option value="1">Solo principal</option>
          <option value="0">Solo no principal</option>
        </select>
  ...
  <div class="relative mx-auto mt-8 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
    <div class="bg-[var(--c-surface)] rounded-2xl border border-[var(--c-border)] overflow-hidden">
      <div class="px-6 py-4 border-b border-[var(--c-border)]">
        ...
        <p class="text-xs text-[var(--c-muted)] mt-1">La opcion principal solo permite una agencia activa en toda la base de datos.</p>
      </div>
      ...
      <form id="agency-form" class="p-6 space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label for="agency-phone" class="block text-sm font-medium text-[var(--c-text)] mb-1">Telefono</label>
        ...
        <div>
          <label class="flex items-center">
            <input type="checkbox" id="agency-is-primary" class="rounded border-[var(--c-border)] text-[var(--c-primary)] focus:ring-[var(--c-primary)]">
            <span class="ml-2 text-sm font-medium text-[var(--c-text)]">Marcar como principal</span>
          </label>
          <p class="text-xs text-[var(--c-muted)] mt-1">Si activas esta opcion, cualquier otra agencia principal sera desmarcada automaticamente.</p>
        </div>
        ...
        <div>
          ...
          <p class="text-xs text-[var(--c-muted)] mt-1">Opcional. Si lo llenas, debe ser JSON valido.</p>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (!API_TOKEN) {
    showError('Autenticacion requerida', 'No se encontro un token de acceso valido. Inicia sesion nuevamente.');
    return;
  }
  ...
  document.getElementById('filter-primary').addEventListener('change', () => loadAgencies(1));
  ...
  async function loadAgencies(page = 1) {
    ...
    const primary = document.getElementById('filter-primary').value;

    const p = new URLSearchParams();
    p.set('page', String(page));
    p.set('per_page', '15');
    p.set('order', 'is_primary');
    p.set('sort', 'desc');
    if (search) p.set('search', search);
    if (primary !== '') p.set('is_primary', primary);

    try {
      const payload = await apiFetch(`${API_BASE}/agencies?${p.toString()}`);
      if (payload?.success) {
  ...
  function badgePrimary(v) {
    if (v) {
      return '<span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-100">Principal</span>';
    }
    return '<span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-[var(--c-elev)] text-[var(--c-muted)] border border-[var(--c-border)]">Secundaria</span>';
  }
  ...
  function renderAgencies(agenciesData) {
    ...
    agenciesData.data.forEach(agency => {
      ...
      el.innerHTML = `
        <div class="flex items-center gap-4 min-w-0">
          ...
          <div class="min-w-0">
            <div class="flex flex-wrap items-center gap-2">
              ...
              ${badgePrimary(!!agency.is_primary)}
            </div>
        ...
        <div class="flex flex-wrap items-center gap-2 justify-end">
          <button
            class="mark-primary-btn px-3 py-1 text-sm rounded-lg transition ${agency.is_primary ? 'bg-green-600 text-white cursor-default' : 'bg-[var(--c-surface)] border border-[var(--c-border)] text-[var(--c-text)] hover:bg-[var(--c-elev)]'}"
            data-id="${escapeHtml(agency.id)}"
            ${agency.is_primary ? 'disabled' : ''}>
            ${agency.is_primary ? 'Principal actual' : 'Marcar principal'}
          </button>
          <button class="edit-agency-btn px-3 py-1 text-sm bg-[var(--c-primary)] text-[var(--c-primary-ink)] rounded-lg hover:opacity-95 transition"
            ...
            data-is-primary="${agency.is_primary ? '1' : '0'}"
    ...
    container.querySelectorAll('.mark-primary-btn').forEach(btn => {
      btn.addEventListener('click', (e) => setAgencyPrimary(e.currentTarget.dataset.id));
    });
  }
  ...
  function renderPagination(agenciesData) {
    ...
    pageInfo.textContent = `Pagina ${agenciesData.current_page} de ${agenciesData.last_page}`;
  ...
  function openAgencyModal(id = null, name = '', accountOwner = '', logoUrl = '', phone = '', email = '', isPrimary = false, rawPayload = null) {
    ...
    const primaryField = document.getElementById('agency-is-primary');
    ...
    if (id) {
      ...
      primaryField.checked = !!isPrimary;
  ...
  async function saveAgency(e) {
    ...
    const isPrimary = document.getElementById('agency-is-primary').checked;
    ...
    if (rawText) {
      try {
        ...
        showError('JSON invalido', 'El campo raw payload debe ser JSON valido.');
        return;
      }
    }
  ...
  async function setAgencyPrimary(id) {
    try {
      const payload = await apiFetch(`${API_BASE}/agencies/${id}/primary`, { method: 'PATCH' });
      if (payload?.success) {
  ...
  async function deleteAgency(id) {
    if (!confirm('Estas seguro de que quieres eliminar esta agencia?')) return;
